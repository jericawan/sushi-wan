...
#include <sys/wait.h>
#include "Sushi.hh"
//received help from Damir and Chat gpt  
...
int Sushi::spawn(Program *exe, bool bg)
{
  ...
  if (bg==/*true*/false) 
  {
    ...
    if(waitpid(pid, &status, 0) != pid) {
      ...
      putenv(new std::string("?"), new std::string(std::to_string(status)));
  } 
  else
  {
    putenv(new std::string("?"), new std::string("0"));
  }
